 ...
 * A goroutine is a lightweight thread managed by the Go runtime.
 ...
 go f(x,y,z)
 ```
starts a new go routine running ``f(x,y,z)``.
The evaluation of ``f,x,y`` and ``z`` happens in the current goroutine and the exectution of ``f`` happens in the new goroutine.
